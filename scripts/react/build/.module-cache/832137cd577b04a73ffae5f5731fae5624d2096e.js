...
var g_bookData = {
	...
	set: function(bookData){
		g_bookData.index = bookData.index;
		g_bookData.book = bookData.book;
		g_bookData.initialized = true;
	},
	getIndex: function(){
		if(!g_bookData.initialized)
			throw "g_bookData not initialized"
		return g_bookData.index;
	},
	getAllBook: function(){
		if(!g_bookData.initialized)
			throw "g_bookData not initialized"
		return g_bookData.book;
	},
	getBookToShow: function(searchTerm){
		...
		for(var c=0; c<all_book.length; c++){
			var found = false;
			for( key in all_book[c] ){
				if(all_book[c][key].indexOf(searchTerm) != -1){
					found = true;
					break;
				}
			}
