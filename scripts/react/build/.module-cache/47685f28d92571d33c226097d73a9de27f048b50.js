...
var g_currentBook = {
	value:-1,
	get: function getCurrentBook(searchTerm){
		if(g_currentBook.value < 0)
			throw "No book selected";
		var all_book = bookData.getAllBook();
		for (var c = all_book.length - 1; c >= 0; c--) {
			if(all_book[c].id == g_currentBook.value){
				book_search_match(searchTerm, all_book[c]);
				break;
			}
		};
		return g_currentBook.value;
	},
	getNoCheck: function (searchTerm){
		try{
			return g_currentBook.get(searchTerm);
		}catch(e){
...
function re_render_book_table(searchTerm){
	React.render(
		React.createElement(BookTable, {
			...
			selectedBook: g_currentBook.getNoCheck(searchTerm), 
