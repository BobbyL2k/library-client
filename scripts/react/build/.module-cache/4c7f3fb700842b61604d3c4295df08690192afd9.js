BookTableContent = React.createClass({displayName: "BookTableContent",
	render: function(){
		...
		return (
			React.createElement("tr", {onClick: this.props.onClick}, 
				columns
			)
			);
	}
...
BookTable = React.createClass({displayName: "BookTable",
	render: function(){
		...
		for (var c = 0; c < list.length; c++) {
			reactList[c] = (React.createElement(BookTableContent, {key: c, book: list[c], index: index, onClick: this.props.onClick}));
		};
		return (
