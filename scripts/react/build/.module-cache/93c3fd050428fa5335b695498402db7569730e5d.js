...
function re_render_book_table(searchTerm){
	React.render(
		React.createElement(BookTable, {
			...
			selectedBook: g_currentBook.getCurrentBookIdNoCheck(searchTerm), 
			onClickBook: g_currentBook.setId}),
		document.getElementById('book-table-container')
	);
	var currentBook = g_currentBook.getCurrentBook();
	React.render(
		React.createElement("div", null, 
		React.createElement(EventButton, {className: "btn-primary", disabled: currentBook.count > 0, buttonName: "ยืมหนังสือ"}), 
		React.createElement(EventButton, {className: "btn-danger", disabled: currentBook.count < currentBook.maxCount, buttonName: "คืนหนังสือ"})
		),
